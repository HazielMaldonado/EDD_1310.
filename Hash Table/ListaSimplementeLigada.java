...
public class ListaSimplementeLigada<T> {
    ...
    public void agregarDespues(T referencia, T valor){
        ...
        while (curr_nodo.getSiguiente() != null && curr_nodo.getDato() != referencia){
    ...
    public T getValor(T valor){
        ...
        while(iteradorNodo != null){
            if(valor.equals(iteradorNodo.getDato())){
                return iteradorNodo.getDato();
            }
            iteradorNodo = iteradorNodo.getSiguiente();
        }
        return null;
    }
    public void Eliminarvalor(T eliminar){
        if(estaVacia()) return;
        Nodo<T> anteriorNodo = null;
        Nodo<T> iteratorNode = head;
        while(iteratorNode.getSiguiente()!= null && !eliminar.equals(iteratorNode.getDato())){
            anteriorNodo = iteratorNode;
    ...
    public void actualizar (T a_buscar, T valor){
        ...
        Nodo<T> curr_nodo = head;
        while(curr_nodo != null && curr_nodo.getDato() != a_buscar){
            curr_nodo = curr_nodo.getSiguiente();
            
        }
        if(curr_nodo.getDato() != a_buscar)return;
        curr_nodo.setDato(valor);
